...
def generate_prophet_files(uploaded_file, end_date):
    ...
    download_file["Sales"] = fut["yhat"][last_idx:]
    download_file["Date"] = pd.to_datetime(download_file["Date"])
    st.line_chart(download_file, x="Date", y="Sales")
...
#     df["Date"] = pd.to_datetime(df["Date"])
    ...
    forecast_df["Date"] = x
    forecast_df["Sales"] = preds
    forecast_df = forecast_df.reset_index(drop=True)
    forecast_df["Date"] = pd.to_datetime(forecast_df["Date"])
    fig = plot_time_series(timesteps=forecast_df["Date"], values=forecast_df["Sales"])
    st.line_chart(forecast_df, x="Date", y="Sales")
